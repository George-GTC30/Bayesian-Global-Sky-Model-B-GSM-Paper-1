...
fit_curved=[True,True]#[False,False,False]
#fit_curved=[False,False,False]
...
map_prior_variance_spec_index = -2.6#-2.8#-4
map_prior_variance_f0 = 408#fixed_f0
map_prior_std=300#295#1000
...
if calibrate == True:
    if calibrate_all == True:
        freqs_to_calibrate = np.array([True,True,True,True,True,True,True,True,True]) #calibrate all the maps
    ...
    if calibrate_all_but_45_150 == True:
        freqs_to_calibrate = np.array([False,True,True,True,True,True,False,True,True]) #calibrate all the maps except 45 and 150
        freqs_to_fit_noise = np.array([False,False,False,False,False,False,False,fit_haslam_noise,fit_haslam_noise])
...
print (np.diag(np.nanmean(inv_noise_mats,axis=0))/np.diag(np.nanmean(inv_noise_mats,axis=0))[-1])
...
print ("least squares fit to ",res.x)
print ("diag elements (using this spec) are",np.exp(res.x[1])*(np.array(freqs)/map_prior_variance_f0)**res.x[0]/(np.exp(res.x[1])*(408/map_prior_variance_f0)**res.x[0]))
print ("diag elements (using our prior assumption) are",(np.array(freqs)/map_prior_variance_f0)**(-2*map_prior_variance_spec_index))

print ("fitting a curved power spectrum to the noise covars")
...
for i in range(len(freqs)):
    ...
    plt.axes(ax)
    hp.mollview(map_i,title="mock data freq="+str(freqs[i]),hold=True,notext=True,norm="log")



plt.savefig(path+"/mock_sky_maps_for_dataset")
#plt.show()
fig = plt.figure(figsize=(12,16))
for i in range(len(freqs)):
...
#we use uniform errors for EDGES
...
if print_vals_as_calc == True:
    if no_of_comps==2:
            ...
            if no_to_fit_curve==no_of_comps:
                init_vals_for_prior = np.append(np.array([0.49,0.5,0.4,0.55]),0.9*np.ones(nDims-no_of_params_for_spec_mod))
                
            else:
                init_vals_for_prior = np.concatenate((np.array([0.49,0.5]),0.4*np.ones(no_to_fit_curve),0.9*np.ones(nDims-no_of_params_for_spec_mod)))
